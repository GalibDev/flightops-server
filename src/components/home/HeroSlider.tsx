...
export default function HeroSlider(){
  ...
  return <section aria-roledescription="carousel" aria-label="FlightOps highlights" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)} className="relative bg-slate-950 text-white">
    ...
    <div className="container relative grid h-[400px] items-center gap-8 py-6 lg:grid-cols-[1.05fr_.95fr]">
      <div className="max-w-2xl"><div key={active} className="hero-copy"><span className="inline-flex items-center gap-2 rounded-full border border-blue-400/25 bg-blue-500/10 px-4 py-2 text-xs font-black tracking-[.14em] text-blue-300"><Plane size={14}/>{slide.eyebrow}</span><h1 className="mt-4 text-[clamp(36px,4.5vw,54px)] font-black leading-[.98] tracking-[-.055em]">{slide.title}<br/><span className="text-blue-400">{slide.accent}</span></h1><p className="my-4 max-w-xl leading-6 text-slate-300">{slide.description}</p><div className="flex flex-wrap gap-3"><Link className="btn btn-primary" href="/flights">Explore flights <ArrowRight size={18}/></Link><Link className="btn border border-white/20 bg-white/10 text-white backdrop-blur hover:bg-white/20" href={active===2?"/dashboard":"/about"}>{active===2?"View dashboard":"Discover FlightOps"}</Link></div><div className="mt-4 hidden flex-wrap gap-5 text-sm font-bold text-slate-300 xl:flex"><span className="flex items-center gap-2"><Check className="text-emerald-400" size={17}/>Verified schedules</span><span className="flex items-center gap-2"><Check className="text-emerald-400" size={17}/>Secure access</span><span className="flex items-center gap-2"><Check className="text-emerald-400" size={17}/>Clear fares</span></div></div></div>
