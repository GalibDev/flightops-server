...
export async function connectDB(){if(!MONGODB_URI) return null;if(cached!.conn)return cached!.conn;if(!cached!.promise)cached!.promise=mongoose.connect(MONGODB_URI);cached!.conn=await cached!.promise;return cached!.conn;}
